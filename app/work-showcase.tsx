...
const groups: Array<{
...
}> = [
  {
    id: 'product',
    title: '产品与体验设计',
    english: 'PRODUCT & EXPERIENCE DESIGN',
  },
  {
    id: 'service',
    title: '服务与场景设计',
    english: 'SERVICE & CONTEXT DESIGN',
  },
  {
    id: 'visual',
    title: 'UI、视觉与新技术实践',
    english: 'UI, VISUAL & EMERGING PRACTICE',
  },
];
...
export function WorkShowcase() {
  ...
  useEffect(() => {
    const syncFilterFromUrl = () => {
      const value = new URL(window.location.href).searchParams.get('work');
      const nextFilter = filters.some(([id]) => id === value)
        ? (value as Filter)
        : 'all';
  ...
  function selectFilter(nextFilter: Filter) {
    ...
    if (nextFilter === 'all') url.searchParams.delete('work');
    else url.searchParams.set('work', nextFilter);
    ...
    <section id="work" className="work-section wrap">
      <div className="section-heading-unified work-intro" data-reveal>
        ...
        <div className="work-filter-wrap">
          <fieldset className="work-filter">
            <legend className="sr-only">筛选作品</legend>
      ...
      {groups.map((group) => (
        <section
          className={`work-group work-group-${group.id}`}
          hidden={filter !== 'all' && filter !== group.id}
          key={group.id}
        >
          ...
          {group.id !== 'visual' && (
            <div
              className={`project-grid ${group.id === 'product' ? 'featured-grid' : 'service-grid'}`}
            >
              {groupProjects[group.id].map((slug, index) => (
                <ProjectCard
                  slug={slug}
                  key={slug}
                  featured={group.id === 'product' && index === 0}
                />
              ))}
            </div>
          )}
          ...
          {group.id === 'visual' && (
            <div className="visual-practice-grid">
              ...
              <VisualPlaceholder
                ...
                type="INTERNSHIP / UI"
                title="元稚产品 UI"
                description="产品界面、组件与迭代片段"
              />
              <VisualPlaceholder
                number="02"
                short="3D"
                type="INTERNSHIP / 3D / MOTION"
                title="芒果TV 综艺视觉"
                description="建模、渲染与动态合成片段"
              />
              <VisualPlaceholder
                number="03"
                short="EXP."
...
function ProjectCard({
  ...
  return (
    <a
      className={`project-card ${project.slug}${featured ? ' is-featured' : ''}${visual ? ' visual-case-card' : ''}`}
      href={`/work/${project.slug}`}
      data-reveal="project"
    >
      <div className={`project-image transition-${project.slug}`}>
        <Image
      ...
      <div className="project-meta">
        <span>{projectFacts[project.slug]}</span>
...
function VisualPlaceholder({
  ...
  return (
    <article className="visual-work-card" data-reveal>
      <div className="visual-work-slot" aria-hidden="true">
        <span>{number}</span>
        <strong>{short}</strong>
      </div>
      <div>
        <span className="visual-work-type">{type}</span>
        <h3>{title}</h3>
        <p>{description}</p>
      </div>
    </article>
