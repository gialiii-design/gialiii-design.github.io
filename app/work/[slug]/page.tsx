...
export default async function Project({params}: {params: Promise<{slug: string}>}) {
  ...
  return <>
    ...
    <main id="main" className="wrap">
      <section className="case-top" data-reveal>
      ...
      <div className={`case-cover transition-${p.slug}`} data-reveal><img src={projectAsset(p.slug)} alt={p.title + '项目展示'} width="1920" height="1080"/></div>
      <section className="case-info" data-reveal><div className="eyebrow overview-label">PROJECT OVERVIEW</div><p className="case-focus">{p.focus}</p><div><h2>从问题出发</h2><p>{p.challenge}</p><h2>设计方向</h2><p>{p.approach}</p></div></section>
      {sections ? <>
        <nav className="case-toc" aria-label="项目内容目录" data-reveal><span>项目内容</span>{sections.map(s => <a href={`#${s.id}`} key={s.id}>{s.title} ↓</a>)}</nav>
        <div className="case-chapters">{sections.map((s, i) => <section id={s.id} className="case-chapter" key={s.id} data-reveal>
        ...
        <section className="process" aria-label="设计思路" data-reveal>{steps.map(([title, body], i) => <div key={title}><span>0{i + 1}</span><h3>{title}</h3><p>{body}</p></div>)}</section>
      </>}
      <p className="case-note" data-reveal>{p.note}</p>
      <a className="case-next" href={`/work/${next.slug}`} data-reveal><div><div className="eyebrow">{next.slug === 'aigc' ? '项目延伸 / EXPLORATION' : '继续浏览 / NEXT PROJECT'}</div><h2>{next.title}</h2></div><span aria-hidden="true">↗</span></a>
    </main>
    <Footer/>
  </>;
}
