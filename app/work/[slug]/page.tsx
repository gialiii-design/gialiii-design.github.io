...
import {chapters, lvSteps, prototypeUrl} from '../../case-content';
...
export default async function Project({params}: {params: Promise<{slug: string}>}) {
  ...
  const steps = slug === 'lv' ? lvSteps : p.steps;
  return <>
    <Header/>
    <main id="main" className="wrap">
      <section className="case-top">
        ...
        <div className="case-actions">
          <span className="pdf-placeholder" aria-disabled="true">查看项目 PDF ↗ <small>待更新</small></span>
      ...
      {sections ? <>
        ...
        <div className="case-chapters">{sections.map((s, i) => <section id={s.id} className="case-chapter" key={s.id}>
          ...
          <div className="case-gallery">{s.pages.map(([page, caption]) => <figure key={page}>
            <a href={`/projects/${slug}-page-${String(page).padStart(2, '0')}.webp`} target="_blank" rel="noopener noreferrer" aria-label={`查看大图：${caption}`}><img src={`/projects/${slug}-page-${String(page).padStart(2, '0')}.webp`} alt={caption} width="1920" height="1080" loading="lazy" decoding="async"/></a>
